<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:79:"D:\phpStudy\WWW/application/datadictionary\view\test_type_management\index.html";i:1530610082;}*/ ?>
<!DOCTYPE HTML>
<html>
    ...
    <div class="cl pd-5 bg-1 bk-gray">
				<span class="l">
                    <a class="btn btn-primary radius" href="javascript:;"
                       onclick="layer_show('添加测试类型','__DataDictionaryURL__/test_type_management/create_page','800','320')">
                        <i class="Hui-iconfont">&#xe600;</i> 添加测试类型
                    </a>
                    <a class="btn btn-warning radius" href="javascript:;" onclick="openEditPage()">
                        <i class="Hui-iconfont">&#xe647;</i>
                        编辑测试类型
                    </a>
                    <a class="btn btn-danger radius" href="javascript:;" onclick="deleteType()">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                        删除测试类型
                    </a>
                </span>
...
<script type="text/javascript">
    ...
    function init(userId) {
        $table = $('#table').bootstrapTable({
            url: '__DataDictionaryURL__/test_type_management/getTypeList',
            method: 'get',
            cache: false,
    ...
    function openEditPage() {
        ...
        if ((id == 0) || (id == null)) {
            layer.msg("请勾选需要编辑的测试类型！", function () {
            });
            return;
        }
        ...
        layer_show('编辑测试类型', '__DataDictionaryURL__/test_type_management/edit_page?typeId=' + id, '800', '320');
    }
    ...
    function deleteType() {
        ...
        if ((id == 0) || (id == null)) {
            layer.msg("请勾选需要删除的测试类型！", function () {
            });
            return;
        }
        if (creator != '<?php echo $realName; ?>') {
        ...
        layer.confirm('测试类型删除须谨慎，确认要删除吗？', function (index) {
            var usrData = {
                'typeId': id.toString()
            };
            $.ajax({
                data: usrData,
                url: "__DataDictionaryURL__/test_type_management/deleteType",
                type: "post",
    ...
    function operate(value, row, index) {
        var typeId = row.id.toString();
        var remarkTitle = "'查看备注'";
        var remarkUrl = "'__DataDictionaryURL__/test_type_management/view_remark_page?typeId=" + typeId + "'";
        var remarkWidth = "'800'";
